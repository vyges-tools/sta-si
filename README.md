...
lumped total, so cell delay drops on resistive nets (this benefits NLDM too, not
just CCS). Fully offline, no external deps, 41 tests green.
...
The road to sign-off grade builds on the same graph: Ceff iteration to convergence
and full waveform-into-RC convolution (deeper CCS accuracy), then path-based
analysis (PBA) and LVF-grade variation. See
